...
@section('title', 'Редактировать - PHARMA')

@section('content')
    <div class="form-header">
        <a href="{{ route('medicines.show', $medicine) }}" class="back-link">
    ...
    <div class="form-card">
        <form action="{{ route('medicines.update', $medicine) }}" method="POST" class="medicine-form">
            @csrf
            @method('PUT')
            ...
            <div class="form-group">
                <label for="image_url">
                    <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                        <rect x="2" y="4" width="14" height="10" rx="1" stroke="var(--gold)" stroke-width="1.5"/>
                        <path d="M2 7L6 11L10 7L16 13" stroke="var(--gold)" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                    URL изображения
                </label>
                <input type="url" id="image_url" name="image_url" value="{{ old('image_url', $medicine->image_url) }}" placeholder="https://example.com/image.jpg">
                @error('image_url')
                    <div class="error-message">{{ $message }}</div>
                @enderror
                <small style="color: rgba(255, 255, 255, 0.5); font-size: 0.85em; margin-top: 5px; display: block;">Вставьте ссылку на изображение из интернета</small>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea id="description" name="description" rows="4">{{ old('description', $medicine->description) }}</textarea>
                @error('description')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" id="price" name="price" step="0.01" min="0" value="{{ old('price', $medicine->price) }}" required>
                    @error('price')
                        <div class="error-message">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="quantity" name="quantity" min="0" value="{{ old('quantity', $medicine->quantity) }}" required>
                    @error('quantity')
                        <div class="error-message">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    <input type="date" id="expiry_date" name="expiry_date" value="{{ old('expiry_date', $medicine->expiry_date ? $medicine->expiry_date->format('Y-m-d') : '') }}">
                    @error('expiry_date')
                        <div class="error-message">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="form-actions">
                ...
                <a href="{{ route('medicines.show', $medicine) }}" class="btn btn-secondary">Отмена</a>
            </div>
        </form>
    </div>
@endsection
